...
# -*- coding: utf-8 -*-
"""
Created on Wed Jan 23 16:01:17 2019
...
from plot import Polar_plot, Three_D_surface_plot, XY_plot, Wireframe_plot, Bar_plot, make_pretty
 

#Function returns dictionaries of figures, axes and all data corresponding to each subplot plotted after click

def cii_extract_from_click(CLICK_FILENAME,ALL_DATA_DICT,DATA_EXTRACTED_BASELINE,QUANTITIES_TO_BE_PLOTTED,PLOT_LIMITS,PLOT_LIMITS_DELTA,DISPLAY_SETTINGS,CLICK_FIG_DETAILS):
    #Data corresponding to Bo-105 only
    Linear_twist=-8
    Radius=4.912        

####################################################################################################
####    Extracting general information presumed to be same for 'CLICK_FILENAME' and Baseline    ####            #this section needs to change, even if baseline and another file have different discretization steps it should be possible to compare them without errors
####################################################################################################
    TIME_STEP=ALL_DATA_DICT[CLICK_FILENAME]['TIME_STEP']  
    EPITCH=float(ALL_DATA_DICT[CLICK_FILENAME]['EPITCH'])
    SENSOR_STATION_BL=ALL_DATA_DICT[CLICK_FILENAME]['SENSOR_STATION_BL']
    AERODYNAMIC_SENSORS_QUANT=ALL_DATA_DICT[CLICK_FILENAME]['AERODYNAMIC_SENSORS_QUANT']
    SENSOR_STATION_BP=ALL_DATA_DICT[CLICK_FILENAME]['SENSOR_STATION_BP']
    SENSOR_STATION_AERO=ALL_DATA_DICT[CLICK_FILENAME]['PANEL_MIDPOINT']
    PSI_HL=(np.arange(0,360+360.0/TIME_STEP[0],360.0/TIME_STEP[0])).tolist()
...
########################################################################################
    if DISPLAY_SETTINGS[0]:
        BLADELOADS=ALL_DATA_DICT[CLICK_FILENAME]['BLADELOADS_OSCILLATORY']
    else:
        BLADELOADS=ALL_DATA_DICT[CLICK_FILENAME]['BLADELOADS']        
    BLADELOADS_HARMONICS=ALL_DATA_DICT[CLICK_FILENAME]['BLADELOADS_HARMONICS']
    PITCH=ALL_DATA_DICT[CLICK_FILENAME]['PITCH']
    PJOINT_HISTORY=ALL_DATA_DICT[CLICK_FILENAME]['PJOINT_HISTORY']
    SENSOR_DATA_AERO=ALL_DATA_DICT[CLICK_FILENAME]['SENSORS_DATA']
    SPEED=ALL_DATA_DICT[CLICK_FILENAME]['SPEED']
    TRIM_CONTROLS = ALL_DATA_DICT[CLICK_FILENAME]['TRIM_CONTROLS']
    ROTOR_POWER_ALL=ALL_DATA_DICT[CLICK_FILENAME]['ROTOR_POWER_ALL']
    ROTOR_FORCES_AND_MOMENTS_ALL=ALL_DATA_DICT[CLICK_FILENAME]['ROTOR_FORCES_AND_MOMENTS_ALL']
    DATA_HUBLOADS = ALL_DATA_DICT[CLICK_FILENAME]['HUBLOADS']
...
######################################################################################################################################## 
####    Getting data corresponding to DATA_EXTRACTED_BASELINE assuming that input file parameters are the same as CLICK_FILENAME    ####
######################################################################################################################################## 
    if DISPLAY_SETTINGS[0]:
...
##################################################################################################################    
    All_possible_figs_post_click={CLICK_FILENAME:DISPLAY_SETTINGS[1],'BASELINE':DISPLAY_SETTINGS[3],CLICK_FILENAME+'_MINUS_BASELINE':DISPLAY_SETTINGS[2]}
    FIG={}
    GS={}
    AX={}
    ...
    for KEY, VALUE in QUANTITIES_TO_BE_PLOTTED.items():
        ...
        if KEY is not 'ADDITIONAL_DATA':    
            for figurename in FIG:    DATA_EXTRACTED_FOR_WIDGETS_PLOT[figurename][KEY],DATA_WIDGETS_COMMON[figurename][KEY]={},{}

                
                          ########################################################################################              
                          ####    Printing the XY/Bar plots corresponding to Plot_from_click_HUBLOAD_SENSORS  ####  
                          ######################################################################################## 
                    ...
                    if Value:                       #if there exist elements in the list corresponding to the value associated with 'Key'
                        ...
                        for quantity in Value:      #iterating thru the elements in the list
                            ...
                            if 'HARM' in Key:
                                ...
                                Hubload_harmonics={CLICK_FILENAME:DATA_HUBLOADS[Key][quantity],CLICK_FILENAME+'_MINUS_BASELINE':Data_hubloads_delta_harmonics,'BASELINE':DATA_HUBLOADS_BASELINE[Key][quantity]}            
                                for figurename in FIG:
                                    DATA_PLOTTED_POST_CLICK[figurename][KEY][Key]={}
                                    AX[figurename].append(FIG[figurename].add_subplot(GS[figurename][l]))
                                    Bar_plot.bar_plot(FIG[figurename],AX[figurename][l],Fig_title,Hubload_harmonics[figurename],Legend,Xlabel,Ylabel,Xticklabels)
                                ...
                                Hubload={CLICK_FILENAME:DATA_HUBLOADS[Key][quantity],CLICK_FILENAME+'_MINUS_BASELINE':Data_hubloads_delta,'BASELINE':DATA_HUBLOADS_BASELINE[Key][quantity]}            
                                for figurename in FIG:
                                    AX[figurename].append(FIG[figurename].add_subplot(GS[figurename][l]))
                                    Limits=[]
                                    if Key in PLOT_LIMITS and quantity in PLOT_LIMITS[Key]: Limits=PLOT_LIMITS[Key][quantity][0:2]
                                    if Key in PLOT_LIMITS_DELTA and quantity in PLOT_LIMITS[Key] and figurename==CLICK_FILENAME+'_MINUS_BASELINE': Limits=PLOT_LIMITS[Key][quantity][0:2]
                                    XY_plot.xy_plot(FIG[figurename],AX[figurename][l],Fig_title,PSI_HL,Hubload[figurename],Legend,Xlabel,Ylabel,Limits)
                                    DATA_PLOTTED_POST_CLICK[figurename][KEY][Key][quantity]=Hubload[figurename]

                          ########################################################################################              
                          ####  Printing the XY/Bar plots corresponding to Plot_from_click_BLADELOAD_SENSORS  ####  
                          ######################################################################################## 
        if KEY=='LOAD_SENSORS':
                ...
                if VALUE[key]:                                                 #Checking if there are any bladeload sensors to plot
                    ...
                    for sensor_station in VALUE[key]:                        
                        ...
                        if sensor_station in SENSOR_STATION_BL:
                            ...
                            BLADELOADS_EXTRACTED_ALL={CLICK_FILENAME:BLADELOADS_EXTRACT,CLICK_FILENAME+'_MINUS_BASELINE':BLADELOADS_EXTRACT_DELTA,'BASELINE':BLADELOADS_EXTRACT_BASELINE}                           
                            for figurename in FIG:
                                AX[figurename].append(FIG[figurename].add_subplot(GS[figurename][l]))
                                Limits=[]
                                if key in PLOT_LIMITS: Limits=PLOT_LIMITS[key][0:2]
                                if key in PLOT_LIMITS_DELTA and figurename==CLICK_FILENAME+'_MINUS_BASELINE': Limits=PLOT_LIMITS_DELTA[key][0:2]
                                XY_plot.xy_plot(FIG[figurename],AX[figurename][l],key,PSI_BL,BLADELOADS_EXTRACTED_ALL[figurename],Legend,Xlabel,Ylabel,Limits)
                            ...
                            BLADELOADS_HARMONICS_EXTRACTED_ALL={CLICK_FILENAME:BLADELOADS_HARMONICS_EXTRACT,CLICK_FILENAME+'_MINUS_BASELINE':BLADELOADS_HARMONICS_EXTRACT_DELTA,'BASELINE':BLADELOADS_HARMONICS_EXTRACT_BASELINE}                           
                            if DISPLAY_SETTINGS[4]:                            #if harmonic content of bladeloads is desired   
                                ...
                                for figurename in FIG:
                                    ...
                                    Ylabel='Magnitude'
                                    AX[figurename].append(FIG[figurename].add_subplot(GS[figurename][l]))
                                    Bar_plot.bar_plot(FIG[figurename],AX[figurename][l],key,BLADELOADS_HARMONICS_EXTRACTED_ALL[figurename],Legend,Ylabel,Xlabel,Xticklabels)
            ...
            BLADELOADS_ALL={CLICK_FILENAME:BLADELOADS,CLICK_FILENAME+'_MINUS_BASELINE':BLADELOADS-BLADELOADS_BASELINE,'BASELINE':BLADELOADS_BASELINE}                           
            for figurename in FIG:
            ...
            for key in VALUE:
                if VALUE[key]:
                    ...
                    if l>=CLICK_FIG_DETAILS[1]*CLICK_FIG_DETAILS[2]:                              #Checking if the grid is full of plots already or not (if it is then further plotting is stopped and the grid size needs to be increased)  
                        ...
                        print('WARNING: It seems you are seeking to plot more results than the grid size allows for. Increase the grid size (using \'Click_grid_rows\' and \'Click_grid_columns\' to see all results!!')
                        return FIG,AX,DATA_PLOTTED_POST_CLICK,DATA_EXTRACTED_FOR_WIDGETS_PLOT,DATA_WIDGETS_COMMON
                    SENSOR_POS_DATA_EXTRACTED_ALL={CLICK_FILENAME:SENSOR_POS_DICT,CLICK_FILENAME+'_MINUS_BASELINE':SENSOR_POS_DICT_DELTA,'BASELINE':SENSOR_POS_DICT_BASELINE}
                    for figurename in FIG:
                        DATA_PLOTTED_POST_CLICK[figurename][KEY][key]={}
                        ...
                        if key in PLOT_LIMITS_DELTA and figurename==CLICK_FILENAME+'_MINUS_BASELINE': Limits,Tick_details=PLOT_LIMITS_DELTA[key][0:2],PLOT_LIMITS_DELTA[key][2:3]
                        Polar_plot.pcolormesh_plot_smooth(FIG[figurename],AX[figurename][l],key,PSI_BP,SENSOR_STATION_BP,SENSOR_POS_DATA_EXTRACTED_ALL[figurename][key],Limits,Tick_details)
                    ...
                    if DISPLAY_SETTINGS[6]:                                    #if Display_wireplots_also=True 
                        ...
                        for figurename in FIG:
                            ...
                            if key in PLOT_LIMITS: Limits=PLOT_LIMITS[key][0:2]
                            if key in PLOT_LIMITS_DELTA and figurename==CLICK_FILENAME+'_MINUS_BASELINE': Limits=PLOT_LIMITS_DELTA[key][0:2]
                            Wireframe_plot.wireframe_plot(FIG[figurename],AX[figurename][l],key,PSI_BP,[float(station)for station in SENSOR_STATION_BP],SENSOR_POS_DATA_EXTRACTED_ALL[figurename][key],Limits)
            for figurename in FIG:                                
            ...
            for key in VALUE:
                if VALUE[key]:   #if values in Plot_from_click_AERO_SENSORS consist of non-empty lists then execute
                    ...
                    if skip==1:
                        ...
                        QUANTITY_EXTRACTED_ALL={CLICK_FILENAME:QUANTITY,CLICK_FILENAME+'_MINUS_BASELINE':QUANTITY-QUANTITY_BASELINE,'BASELINE':QUANTITY_BASELINE}
                        for figurename in FIG:
                            DATA_PLOTTED_POST_CLICK[figurename][KEY][key]={}
                            AX[figurename].append(FIG[figurename].add_subplot(GS[figurename][l],projection='polar'))
                            Limits,Tick_details=[],[]
                            if key in PLOT_LIMITS: Limits,Tick_details=PLOT_LIMITS[key][0:2],PLOT_LIMITS[key][2:3]
                            if key in PLOT_LIMITS_DELTA and figurename==CLICK_FILENAME+'_MINUS_BASELINE': Limits,Tick_details=PLOT_LIMITS_DELTA[key][0:2],PLOT_LIMITS_DELTA[key][2:3]
                            Polar_plot.pcolormesh_plot_smooth(FIG[figurename],AX[figurename][l],key,PSI_AERO,SENSOR_STATION_AERO,QUANTITY_EXTRACTED_ALL[figurename],Limits,Tick_details)
                    ...
                    if DISPLAY_SETTINGS[6]:                                    #if Display_wireplots_also=True 
                        ...
                        for figurename in FIG:
                            ...
                            if key in PLOT_LIMITS: Limits=PLOT_LIMITS[key][0:2]
                            if key in PLOT_LIMITS_DELTA and figurename==CLICK_FILENAME+'_MINUS_BASELINE': Limits=PLOT_LIMITS_DELTA[key][0:2]
                            Wireframe_plot.wireframe_plot(FIG[figurename],AX[figurename][l],key,PSI_AERO,SENSOR_STATION_AERO,QUANTITY_EXTRACTED_ALL[figurename],Limits)
            for figurename in FIG:                                
        ...
        if KEY=='ADDITIONAL_DATA':
            ...
            for Key, Value in VALUE.items():
                if Key=='TRIM_CONTROLS' and Value:     #if the Key is 'TRIM CONTROLS' and there is something in the 'value' associated with (NOTE that this also plots the L/D) 
                    ...
                    TRIM_CONTROLS_AND_LbyD_EXTRACTED_ALL={CLICK_FILENAME:TRIM_CONTROLS_AND_LbyD_EXTRACTED_CLICK_FILENAME,CLICK_FILENAME+'_MINUS_BASELINE':TRIM_CONTROLS_AND_LbyD_EXTRACTED_DELTA,'BASELINE':TRIM_CONTROLS_AND_LbyD_EXTRACTED_BASELINE}                           
                    Legend='CII'
                    Ylabel='Magnitude'
                    Xlabel=''
                    for figurename in FIG:
                        ...
                        AX[figurename].append(FIG[figurename].add_subplot(GS[figurename][l]))
                        tuple(Xticklabels)
                        Bar_plot.bar_plot(FIG[figurename],AX[figurename][l],'TRIM_ANGLES_and_LbyD',TRIM_CONTROLS_AND_LbyD_EXTRACTED_ALL[figurename],Legend,Ylabel,Xlabel,Xticklabels)
                ...
                if Key=='PJOINT_HISTORY' and Value:     #if the Key is 'PJOINT_HISTORY' and the value is True 
                    ...
                    PJOINT_HISTORY_EXTRACTED_ALL={CLICK_FILENAME:PJOINT_HISTORY_EXTRACTED_CLICK_FILENAME,CLICK_FILENAME+'_MINUS_BASELINE':PJOINT_HISTORY_EXTRACTED_DELTA,'BASELINE':PJOINT_HISTORY_EXTRACTED_BASELINE}                           
                    Legend='CII'
                    Ylabel='Pitch [deg]'
                    Xlabel=r'$\psi$'
                    for figurename in FIG:
                        ...
                        if Key in PLOT_LIMITS: Limits=PLOT_LIMITS[Key][0:2]
                        if Key in PLOT_LIMITS_DELTA and figurename==CLICK_FILENAME+'_MINUS_BASELINE': Limits=PLOT_LIMITS_DELTA[Key][0:2]
                        XY_plot.xy_plot(FIG[figurename],AX[figurename][l],Key,PSI_BP,PJOINT_HISTORY_EXTRACTED_ALL[figurename],Legend,Xlabel,Ylabel,Limits)
                        DATA_PLOTTED_POST_CLICK[figurename][KEY][Key]['Xdata']=PSI_BP
                ...
                if Key=='FLAP_DEFLECTION_ANGLE' and Value:  #Quick and dirty, extracting data from info acquired thru AERO_SENSORS
                    ...
                    for figurename in FIG:
                        ...
                        XY_plot.xy_plot(FIG[figurename],AX[figurename][l],Key,Xdata,Ydata,Legend,Xlabel,Ylabel,Limits)
